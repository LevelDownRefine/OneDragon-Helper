...
from utils import get_path_under_root, get_onedragon_yml_path_under_root, get_script_chain_under_onedragon
from onedragon_config_ui import run_config_ui

def copy_all_configs():
    output_dir = get_script_chain_under_onedragon()
    os.makedirs(output_dir, exist_ok=True)
    for i in range(1, 8):
        chain_name = f"{i:02d}.yml"
        input_path = get_path_under_root(chain_name)
...
def copy_python_scripts():
    ...
    for file_name in file_names:
        ...
        output_path = os.path.join(get_script_chain_under_onedragon(), "scripts")
