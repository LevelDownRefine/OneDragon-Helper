...
- `set_config.py`：基类新增类属性 `_game_path_keys`（嵌套键路径）与类方法 `get_game_exe_path()`（不实例化、无写盘副作用）；各子类声明 `_game_path_keys`；新增适配器接口 `get_game_exe_path()`。
...
- 测试：`test_subscript.py` 增 `TestLoadGameConfig`（6 例）；`test_set_config_subclasses.py` 增 `TestGetGameExePath` + `TestGetGameExePathAdapter`（10 例）；`test_gui_widgets.py` 增 `TestScriptItemGameMenu`（3 例）。全量 430 通过，ruff 干净。
- 真实环境验证：7 个游戏脚本全部读出真实路径，python/自定义脚本返回 None。
- **后续全局改造（2026-08-10）**：`get_game_exe_path()` 适配器接口入参从 display_name 改为 script_name（exe 用进程名）；GUI `ScriptItem` 用 `script_name` 属性定位，`_build_game_menu` 传 `script_name`。python 脚本的 `script_name` = display_name，不在 `_CONFIGS` 注册表 → 右键不弹「打开游戏」。
