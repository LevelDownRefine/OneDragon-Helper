...
- 机制类在 `daily.py`：`GenshinDaily`（原神秘境资源筛选，读写沿用基类）、`NoopDaily`（绝区零/崩铁，上游自身已支持）、`Anomaly`（数据在自己段、开关在第二份文件）、`MaaDaily`（粥的 TaskQueue/StagePlan）、`MaaActivityDaily`（活动选项及过期检查）。适配器通过统一的 `_create_daily` 按声明构造日常。
...
日常资源选项由 `Daily.get_task_lists(source)` 持有。菜单把完整日常声明与当前选项的 `source` 交给 `ScriptConfig.get_task_lists(declaration, source)`；适配器只统一构造并委托，不按脚本覆写或反查日常声明。`GenshinDaily` 筛选秘境分类，`MaaActivityDaily` 直接使用自己的配置路径确定客户端。通用 `path / key` 读取放在 `task_source.read_task_source`，供 `Daily` 基类和周常菜单共用。读取菜单不触发配置初始化或写盘。
