...
落点（触发时机）：写路径适配器 `set_config` / `set_weekly_dungeon` / `set_weekly_start_day` 实例化后、写字段前各调一次 `cfg._init_config()`。无 `_template_rel_path` 直接返回、`self._load(allow_missing=True)` 缺失返回——守卫确保无模板或脚本未安装时为空操作。反读适配器（`get_dungeon` 等）一律不触发，保持纯只读。
