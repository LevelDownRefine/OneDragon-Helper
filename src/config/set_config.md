...
落点（触发时机）：写路径适配器 `set_config` / `set_weekly_dungeon` / `set_weekly_start_day` 在实例化后调用 `init_config(script_name)`。无 `_template_rel_path` 直接返回、`self._load(allow_missing=True)` 缺失返回——守卫确保无模板或脚本未安装时为空操作。反读适配器（`get_dungeon` 等）一律不触发，保持纯只读。

| 脚本 | 当前调用 _init_config | 模板 | 说明 |
|------|---------------------|------|------|
| 鸣潮 | 是（no-op，无模板→直接返回） | — | 写路径自动触发，无模板时为空操作 |
| 原神 | 是 | `BGI一条龙.json` | 写路径自动触发，模板对齐补全缺失字段 |
...
2. 设 `_task_key` / `_task_map`，需序列支持则覆盖 `_update_task`（在 `super()._update_task(config, dungeon_name, None)` 后补序列），标准流程不够则覆盖 `set_dungeon`；`_init_config` 已在写路径适配器自动触发，新增脚本无需显式调用；无 `_template_rel_path` 时为空操作。
