...
> 无需适配的日常（绝区零/崩铁）不设 no_op 标志：`NoopDaily` 覆写 `update` 恒返回 False——读盘一次但不落盘，类型检查消失。
