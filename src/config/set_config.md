...
落点（触发时机）：写方法 `set_dungeon` / `set_weekly` / `set_weekly_start_day` / `set_weekly_dungeon` 开头自动调用。无 `_template_rel_path` 直接返回、`self._load(allow_missing=True)` 缺失返回——守卫确保无模板或脚本未安装时为空操作。反读方法（`get_dungeon` 等）一律不触发，保持纯只读。
