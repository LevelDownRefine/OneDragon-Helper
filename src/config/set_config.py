...
class ArknightsConfig(ScriptConfig):
    ...
    def _init_config(self) -> None:
        ...
        main, remaining = (d for d in self._dailies if type(d) is MaaDaily)
    ...
    def get_task_lists(cls, source: dict) -> list[str]:
        ...
        assert source.keys() == {"path"}, "MAA 关卡来源只支持 path"
        readers = set()
        for declaration in get_daily_configs(cls._script_name):
            group = get_field(declaration, "options", cls.display_name, dict)
            if "source" not in group or group["source"] != source:
                continue
