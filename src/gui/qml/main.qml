...
Window {
    ...
    // 窗口本体透明：四角由下方 shell 的遮罩给出真正的半透明像素，边缘才能平滑。
    // 旧实现用 QWindow.setMask(QRegion) 裁窗口形状，而 QRegion 是二值区域（每像素
    ...
    // 不绘制（界面全空）；RHI 尚未就绪时 api 也报 Unknown。两种情形都退化为方角，
    // 界面可用优先——本机与打包版都走 D3D11（无 GPU 时 WARP），不受影响。
    readonly property bool cornerMaskSupported:
    ...
    // 整窗内容（背景三层 + UI 层）都装进 shell：先渲染到底图，再按圆角遮罩的
    // alpha 合成，圆角外是真正的透明像素（含视频与壁纸），而不是被 OS 二值裁掉。
    // 注意不能用 `default property alias` 把 Window 的子项转发进来——Window 的
    // 默认属性 data 来自 C++ 类型，QML 端覆盖不生效（实测子项仍落在 contentItem，
    // shell 恒为空），只能显式嵌套。
    Item {
        id: shell
        objectName: "windowShell"
        anchors.fill: parent
        layer.enabled: root.cornerMaskSupported
        layer.effect: MultiEffect {
            objectName: "cornerMaskEffect"
            maskEnabled: true
    ...
    // 圆角遮罩源：与窗口同尺寸的圆角矩形，只取 alpha 作 mask。挂在 shell 之外，
    // 否则它会被自己的遮罩当成内容渲染。
    Rectangle {
        id: cornerMask
        objectName: "cornerMask"
        width: root.width
        height: root.height
