...
通过主配置（config.yml）定位各子脚本的 config 文件，提供统一的读写接口。
"""
...
def _get_script_root_dir(script_display_name: str) -> str:
    ...
    for script in config_data.get('script_list', []):
        if script.get('display_name') == script_display_name:
            ...
            assert script_path, f"[subscript_utils] config.yml 中 {script_display_name} 的 script_path 为空"
            normalized = script_path.replace('\\', '/')
            return os.path.dirname(normalized)
    assert False, f"[subscript_utils] config.yml 中找不到脚本: {script_display_name}"
...
def get_config_path(script_display_name: str) -> str:
    ...
    assert script_display_name in _CONFIG_REL_PATHS, \
        f"[subscript_utils] 未适配脚本: {script_display_name}"
    ...
    assert os.path.exists(config_path), f"[subscript_utils] config 文件不存在: {config_path}"
    return config_path
...
def load_config(script_display_name: str) -> Any:
    ...
    with open(path, 'r', encoding='utf-8') as f:
        if ext == '.json':
            ...
            return yaml.safe_load(f)
        raise ValueError(f"[subscript_utils] 不支持的 config 格式: {ext}")
