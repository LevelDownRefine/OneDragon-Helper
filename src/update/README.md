...
`update-manifest.json`，与本地清单逐文件比对，只下载变化条目的压缩数据，未变文件
从当前安装复制补齐。因此整包 SHA-256 不再参与校验，改由清单里的逐文件 SHA-256 覆盖；
新增与退役文件由安装事务按新旧清单处理。

远端返回整包（不支持范围请求）、ZIP64、非 deflate 压缩或路径越界时抛
`RemoteUnavailable`，由 `prepare_update()` 静默回退全量；网络与校验失败不回退，避免重复下载。
请求复用同一连接，并按区间合并，使条目相邻时以少数几次请求完成。
