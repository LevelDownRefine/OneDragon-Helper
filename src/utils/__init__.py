...
def get_weekly_yml_path_under_root() -> str:
    ...
    config/weekly_task_list.yml（静态，进 git），不在此文件。
...
def get_daily_task_list_yml_path_under_root() -> str:
    """取得日常任务声明文件路径。"""
    return safe_path_join(get_root_dir(), "config", "daily_task_list.yml")


def get_weekly_task_list_yml_path_under_root() -> str:
    """
    获取根目录下的config/weekly_task_list.yml文件路径。
    ...
    :return: 根目录下的config/weekly_task_list.yml文件路径
    """
