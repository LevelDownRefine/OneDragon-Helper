...
class TestSendMail(unittest.TestCase):
    ...
    def test_build_html_colors_status_pill(self):
    ...
    def test_build_html_uses_gui_dark_palette(self):
        """底色与主文字色取自 GUI 主题：邮件与界面同一套配色。"""
        entries = (_entry("鸣潮", ScriptLogStatus.SUCCESS),)
        html_body = _build_html(_result(entries=entries))
    ...
    def test_build_html_diagnostics_cards(self):
        """报错明细渲染为卡片、失败脚本另附日志尾部，且报错段在日志尾部之前。"""
        entry = _entry("崩铁", ScriptLogStatus.FAILED, errors=("ERROR boom",))
        entry["result"]["log_content"] = "tail line"
        html_body = _build_html(_result(entries=(entry,)))
    ...
    def test_build_html_no_diagnostics_when_all_ok(self):
        """无报错脚本且无失败脚本：整段诊断不渲染（不留空标题）。"""
        entries = (_entry("鸣潮", ScriptLogStatus.SUCCESS, stamina="180"),)
        html_body = _build_html(_result(entries=entries))
    ...
    def test_build_html_counts_badges_without_stale_actions(self):
