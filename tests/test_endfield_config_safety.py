...
设计目的：验证 set_daily_task / prepare_weekly_start_day 不会把副本/周常以外的字段（刷体力、购物、
...
class TestEndfieldConfigSafety(unittest.TestCase):
    ...
    # ---- prepare_weekly_start_day：只允许改 只买不卖（反相） ----
    def test_set_weekly_only_touches_weekly_key(self):
        cfg = EndfieldConfig()

        # 制造差异：把周常开关先拨到错误值，逼 prepare_weekly_start_day 真正落盘
        pre = copy.deepcopy(
            self.store["data/apps/ok-ef/working/configs/DailyTask.json"]
        )
        ...
        with patch("src.config.set_config.is_weekly_start_reached", return_value=True):
            cfg.prepare_weekly_start_day(1)  # 周常启用 ⇒ 只买不卖=false
