...
- 模拟 StarRailConfig.prepare_weekly_start_day 的真实写入（currencywars_enable / echo_of_war_start_day_of_week）
  后，仍保真、注释不丢。
...
class TestGameConfigRoundTrip(unittest.TestCase):
    ...
    def test_starrail_weekly_write_round_trip(self):
        """模拟 StarRailConfig.prepare_weekly_start_day 的真实写入后，仍保真且注释不丢。"""
        cfg = self._load_fixture()
        # 与 set_config.py 中 StarRailConfig.prepare_weekly_start_day 的落盘一致
