...
class TestLauncher(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.weekly_timeouts_yml_path = os.path.join(self.mock_root_dir, "weekly_timeouts.yml")

        # Mock configuration data (config.yml no longer contains weekly_timeouts)
        self.config_data = {
            'script_list': [
                {
        ...
        # Weekly timeouts stored in a separate file, keyed by display_name
        self.weekly_timeouts_data = {
            'Test Script 1': [100, 200, 300, 400, 500, 600, 700]
        }
        with open(self.weekly_timeouts_yml_path, 'w', encoding='utf-8') as f:
            yaml.dump(self.weekly_timeouts_data, f)
    ...
    def test_generate_OneDragon_script_chain_success(self, mock_get_path, mock_get_config_yml, mock_get_wt_path, mock_get_week):
        ...
        self.assertEqual(scripts[1]['run_timeout_seconds'], 50)  # Unchanged since no weekly_timeouts entry
    ...
    def test_generate_OneDragon_script_chain_invalid_timeout_length(self, mock_get_path, mock_get_config_yml, mock_get_wt_path, mock_get_week):
        ...
        # Write weekly_timeouts.yml with wrong timeout list size (6 instead of 7)
        invalid_wt_data = copy.deepcopy(self.weekly_timeouts_data)
        invalid_wt_data['Test Script 1'] = [100, 200, 300, 400, 500, 600]
        with open(self.weekly_timeouts_yml_path, 'w', encoding='utf-8') as f:
            yaml.dump(invalid_wt_data, f)
