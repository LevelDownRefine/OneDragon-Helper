...
class TestResolveWeeklyStart(unittest.TestCase):
    """_resolve_weekly_start：从 ui_state 取周常起始日（1=周一~7=周日），不判断今天。"""
    ...
    def test_missing_weekly_start_returns_none(self):
        ...
        self.assertIsNone(
            _resolve_weekly_start({"ok-ww": {"dungeon": "无音区"}}, "ok-ww")
        )

    def test_returns_weekly_start_value(self):
        """已设置 weekly_start → 返回原值（启用/停用由 set_config 自行判断）"""
        result = _resolve_weekly_start({"ok-ww": {"weekly_start": 4}}, "ok-ww")
    ...
    def test_invalid_weekly_start_raises(self):
        ...
        for bad in (0, 8):
            with self.subTest(bad=bad), self.assertRaises(AssertionError):
                _resolve_weekly_start({"ok-ww": {"weekly_start": bad}}, "ok-ww")
