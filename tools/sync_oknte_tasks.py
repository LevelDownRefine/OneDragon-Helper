...
用法：
    python tools/sync_oknte_tasks.py            # 只检测，输出差异报告
    python tools/sync_oknte_tasks.py --apply    # 检测并自动补齐新增项
...
def _fetch_url(url: str) -> str:
    ...
    try:
        ...
        print(f"[sync_oknte_tasks] 抓取失败 {url}: {exc}")
        sys.exit(2)
...
def _fetch_anomaly_totals() -> dict[str, int]:
    ...
    for prefix, label in _RANGE_LABELS.items():
        ...
        if match is None:
            print(
                f"[sync_oknte_tasks] {label} 上游未找到 {prefix}_ID_RANGE"
                " 定义（结构可能变化）"
            )
            sys.exit(2)
...
def _fetch_hunter_targets() -> list[str]:
    ...
    if match is None:
        print("[sync_oknte_tasks] 上游未找到 HUNTER_TARGETS 定义（结构可能变化）")
        sys.exit(2)
    targets = []
    for const in re.findall(r"TARGET_(\w+)", match.group(1)):
        if const not in name_map:
            print(f"[sync_oknte_tasks] HUNTER_TARGETS 引用未定义的 TARGET_{const}")
            sys.exit(2)
...
def _daily_tasks(data: dict, daily_name: str) -> list[dict]:
...
def _load_numeric() -> dict[str, list[int]]:
    ...
    result = {}
    for task in _daily_tasks(_read_yaml(), "daily_anomaly"):
        if task["display_name"] not in _RANGE_LABELS.values():
            continue
...
def _load_hunter() -> list[str]:
    ...
    return [
        ...
        for option in _daily_tasks(_read_yaml(), _HUNTER_TASK)
    ]
...
def _apply_numeric(upstream: dict[str, int]) -> None:
    ...
    for cat, total in upstream.items():
        task = next(
            d for d in _daily_tasks(data, "daily_anomaly") if d["display_name"] == cat
        )
        existing = {s["physical_name"] for s in task["options"]["values"]}
        for num in range(1, total + 1):
            if num not in existing:
                task["options"]["values"].append(
...
def _apply_hunter(targets: list[str]) -> None:
    ...
    data = _read_yaml()
    options = _daily_tasks(data, _HUNTER_TASK)
    existing = {
...
def main() -> int:
    ...
    if not has_diff:
        print("[sync_oknte_tasks] 无差异")
        return 0

    print(
        "[sync_oknte_tasks] 上游异象界域总数："
        + "、".join(f"{cat}={total}" for cat, total in anomaly_upstream.items())
    )
    print(f"[sync_oknte_tasks] 上游追猎目标：{hunter_upstream}")
    ...
    if not apply:
        print("[sync_oknte_tasks] 检测到差异，未应用（加 --apply 自动补齐）")
        return 1

    _apply_numeric(anomaly_upstream)
    _apply_hunter(hunter_upstream)
    print("[sync_oknte_tasks] 已自动补齐新增项（name 占位/中文名）")
    return 0
